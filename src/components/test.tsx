...
interface GridViewProps extends ScrollViewProps {
  ...
  children?: any;
  deleteComponent?: any;
  setShowDelete?: (item: any) => void
  keyExtractor?: (item: any) => string
  renderItem: (item: any, index?: number, selectedItem?: any, isMoving?: boolean) => JSX.Element
...
const GridView = memo((props: GridViewProps) => {
  ...
  const onPress = (item, index, isSameCell) => {
    ...
    if(selectedItem && !isSameCell) {
      setSelectedItem(undefined);
      setShowDeleteComponent(false);
    } else {
      
      onPressCell && onPressCell(item, index)
    }
    ...
    (value: Item, index: number) => {
      ...
      return isLocked ? (
        ...
        >
          ...
          {/* { (selectedItem && (value.item == selectedItem.item)  && showDeleteComponent) ? props.deleteComponent : null} */}
        </Animated.View>
      )
    },
