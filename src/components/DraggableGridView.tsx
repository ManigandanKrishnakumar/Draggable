...
const GridView = memo((props: GridViewProps) => {
  ...
  useEffect(() => {
    onUpdateGrid();
    onLongPress(data[0], 0, {"x": 0, "y": 0}, true);
  ...
  const onLongPress = useCallback(
    (item: string, index: number, position: Point, isSame?:boolean) => {
      if(isSame) {
        
        
        if (self.animation) return

      // console.log('[GridView] onLongPress', item, index, onBeginDragging)
  ...
  const onEndRelease = useCallback(() => {
    ...
    // setIsMove(false);
    setTimeout(() => setIsMove(false), 100);
    onReleaseCell && onReleaseCell(self.items.map((v) => v.item))
    // setSelectedItem(undefined)
    
  }, [onReleaseCell])

  const onPress = (item, index, isSameCell) => {
    ...
    if(!isSameCell) {
     ...
     (!selectedItem) ? onPressCell && onPressCell(item, index) : null;
    }
  ...
  const _renderItem = useCallback(
        ...
        <Animated.View
          ...
          style={[
            style,
            {
              ...
              opacity: (selectedItem && value.item != selectedItem.item) ? 0.5 : opacity,
            },
          ]}
        >
          <TouchableOpacity
            ...
            onLongPress={() => ( onLongPress(item, index, p, selectedItem ? value.item == selectedItem.item : true)) }
            onPress={() => onPress(item, index,selectedItem ? value.item == selectedItem.item : false)}
          >
            {renderItem(item, index)}
          </TouchableOpacity>
          {(selectedItem && value.item == selectedItem.item  && (isMove)) ?<View style={{height: 20, width: 100, backgroundColor: 'black', position: 'absolute', bottom: -10, left: 50, borderRadius: 10}}>
            <TouchableOpacity style={{flex: 1, justifyContent: 'center', alignItems: 'center'}} onPress={() => {props.onDelete(value.item)
           
            setTimeout(()=> { onPress(item, index, false);}, 500);
            }}>
    ...
    [selectedItem, renderLockedItem, renderItem, numColumns]
  )
